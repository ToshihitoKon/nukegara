...
# Usage: ./sync_dotfiles.sh [to-home|from-home] [--force]

set -e

NUKEGARA_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
HOME_DIR="$HOME"
...
get_dotfiles_list() {
    ...
    done < <(find "$NUKEGARA_DIR" -mindepth 1 -maxdepth 1 -name ".*" -type f -print0)
    
    # Pattern 2: .config/[appname] directories
    if [[ -d "$NUKEGARA_DIR/.config" ]]; then
        while IFS= read -r -d '' dir; do
            local app_name=$(basename "$dir")
            dotfiles+=(".config/$app_name")
        done < <(find "$NUKEGARA_DIR/.config" -mindepth 1 -maxdepth 1 -type d -print0)
    fi
...
usage() {
    echo "Usage: $0 [to-home|from-home] [--force]"
    echo ""
    echo "Commands:"
    echo "  to-home    Copy files from nukegara repository to home directory"
    echo "  from-home  Copy files from home directory to nukegara repository"
    echo ""
    echo "Target files:"
    ...
    echo ""
    echo "Examples:"
    echo "  $0 to-home              # Show diff and ask for confirmation before copying to ~"
    echo "  $0 from-home --force    # Copy from ~ to nukegara without confirmation"
}
...
main() {
    ...
    case "$direction" in
        "to-home")
            src_dir="$NUKEGARA_DIR"
            dst_dir="$HOME_DIR"
            echo -e "${BLUE}Copying from nukegara repository to home directory${NC}"
            ;;
        "from-home")
            src_dir="$HOME_DIR"
            dst_dir="$NUKEGARA_DIR"
            echo -e "${BLUE}Copying from home directory to nukegara repository${NC}"
            ;;
        *)
